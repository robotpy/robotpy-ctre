...
def test_talon_set5(ctre, talon, cdata):
    ...
    ctre.TalonSRX.set(
        talon,
        talon.ControlMode.Current,
        1.1,
        talon.DemandType.ArbitraryFeedForward,
        0.4,
    )
    assert cdata["control_mode"] == talon.ControlMode.Current
    assert cdata["pid0_target"] == 1100
...
def test_basemotorcontroller_configGetParameter(
    talon, ctre, cdata, param_name, cdata_key, value, slot
):
...
@pytest.mark.parametrize("param_name, slot, cdata_key, value", config_data)
def test_basemotorcontroller_configSetParameter(
    talon, ctre, cdata, param_name, cdata_key, value, slot
):
    param = ctre.ParamEnum[param_name]
...
def test_basemotorcontroller_getErrorDerivative(talon, cdata):
    cdata["pid1_errorDerivative"] = 50.0
    assert talon.getErrorDerivative(1) == 50.0
...
def test_basemotorcontroller_limitSwitches(ctre, hal_data, talon, cdata):
    ...
    talon.configForwardLimitSwitchSource(
        talon.LimitSwitchSource.RemoteTalonSRX,
        talon.LimitSwitchNormal.NormallyOpen,
        2,
        0,
    )
...
@pytest.mark.parametrize(
    "initial_pos, initial_vel, expected_vel, expected_pos",
    [
        (0, 0, 2.457, 0.02457),
        (1, 0, 2.457, 1.02457),
        (40960, 0, 0, 4096 * 10),
        ...
        (40878, 600, 600 - 2.457, 40878 + 5.97543),
        (40878, -60, -60 + 2.457, 40878 - 0.57543),
        (41041, 60, 60 - 2.457, 41041 + 0.57543),
        (-40878, 1300, 1300 - 2.457, -40878 + 12.97543),
        (90878, -1300, -1300 + 2.457, 90878 - 12.97543),
    ],
)
def test_motion_magic_next_target(
    talon, cdata, initial_pos, initial_vel, expected_vel, expected_pos
):
...
def test_calculate_pid1(talon, cdata):
    ...
    assert_is_almost_equal(1.0, cdata["value"])
